...
from mytunes.processors.tagger.values._collection import MinValue, MaxValue, CountValue, SumValue
...
class TestMinValue(BaseModelTester):
    ...
    def test_get_value_does_not_fail_on_missing(self, tracks: list[Track], faker: Faker):
...
class TestSumValue(BaseModelTester):
    @pytest.fixture
    def model(self) -> SumValue:
        return SumValue(field="track.number")

    def test_get_value(self, tracks: list[Track]):
        model = SumValue(field="track.number")
        assert model.get(tracks) == sum(track.track.number for track in tracks)
    ...
    def test_get_value_does_not_fail_on_missing(self, tracks: list[Track], faker: Faker):
        ...
        model = SumValue(field="track.number")
        assert model.get(tracks) == sum(track.track.number for track in tracks if isinstance(track.track, Position))


class TestCountValue(BaseModelTester):
    @pytest.fixture
    def model(self) -> CountValue:
        return CountValue()

    def test_get_value(self, tracks: list[Track]):
        model = CountValue()
        assert model.get(tracks) == len(tracks)
