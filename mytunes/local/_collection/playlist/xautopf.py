...
class _XMLCondition(_XMLBaseModel):
    ...
    value: Annotated[list[str], _XMLAttributeField(), TO_LIST] = Field(default_factory=list)
    ...
    def _validate_field_is_mapped(cls, field: str) -> str:
        if field not in cls.name_field_map:
            raise MyTunesValidationError(f"Unrecognised condition field name: {field}")
        return field

    @model_validator(mode="after")
    def _validate_only_and_either_or_set(self) -> Self:
        if self.and_conditions is not None and self.or_conditions is not None:
            raise MyTunesValidationError("Condition can only have either 'And' or 'Or' set, not both.")
        return self
...
class _XMLDisplayField(_XMLBaseModel):
    ...
    def _validate_code_is_mapped(code: int) -> int:
        if code not in _XMLCondition.code_name_map:
            raise MyTunesValidationError(f"Unrecognised display field code: {code}")
        return code
...
class _XMLDisplayGroup(_XMLBaseModel):
    ...
    field: Annotated[list[_XMLDisplayField], TO_LIST] = Field(
        default=(
...
class _XMLSortBy(_XMLBaseModel):
    ...
    def _validate_field_is_mapped(code: int) -> int:
        if code not in _XMLCondition.code_name_map:
            raise MyTunesValidationError(f"Unrecognised sort field code: {code}")
        return code
    ...
    def parse_sorter(self, sorter: ItemSorter) -> Self:
        ...
        if len(sorter.sort_fields) > 1:
            raise MyTunesValueError("Only use this sorter for single-field sorts.")
        ...
        if field_code is None:
            raise MyTunesValueError(f"Field code mapping not found for field: {field}")
...
class _XMLDefinedSort(_XMLBaseModel):
    ...
    def parse_sorter(self, sorter: ItemSorter) -> Self:
        """Parse the given ``sorter`` into this model."""
        if len(sorter.sort_fields) <= 1:
            raise MyTunesValueError("Only use this sorter for multi-field sorts.")
        ...
        if unknown_fields:
            raise MyTunesValueError(f"Field code mapping not found for fields: {', '.join(unknown_fields)}")
        ...
        if sort_id is None:
            raise MyTunesValueError(f"No sort defined for the fields: {fields}")
