...
class MusicBee(LocalLibrary, IsReadableFile, IsWriteableFile, IsLocalFile, metaclass=make_cls()):
    ...
    async def save(self, dry_run: bool = False) -> dict[str, Any]:
        """
    ...
    async def _playlists_to_xml(self, xml: dict[str, Any], tracks: dict[Path, int]) -> dict[int, dict[str, Any]]:
        ...
        for pl_id, pl in enumerate(self.playlists, max_playlist_id + 1):
    ...
    def _generate_persistent_id(value: str | Path | None = None, persistent_id: str | None = None) -> str:
        ...
        if (length := len(persistent_id)) > 16:
            raise MyTunesValueError(f"Persistent ID is >16-characters in length ({length=}): {persistent_id}")
