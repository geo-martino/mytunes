...
class PlaylistEndpoints[AT: Authoriser, UT: URI, RT: RemoteMutablePlaylist](RemoteEndpoints[AT, UT, RT]):
    async def extend_tracks(self, playlist: RT) -> None:
        ...
        await self._extend_items_from_cursor(
            items=playlist.tracks, cursor=playlist.cursor, path=self._extend_path, kind="track"
        )
